...
const SITE_BUILD_VERSION = 'ecosystems-app-88';
...
async function fetchAppJson(path, signal) {
  const versionedPath = `${path}${path.includes('?') ? '&' : '?'}v=ecosystems-app-88`;
...
function App() {
  ...
  useEffect(() => {
    ...
    if (isGameApp) {
      ...
      appleIcon?.setAttribute('href', '/assets/monerge-icon-512.png?v=ecosystems-app-88');
      favicon?.setAttribute('href', '/assets/monerge-icon-512.png?v=ecosystems-app-88');
      return;
    }
    document.title = 'Gerry Stephen · Business, Web3, and the Iglu';
    appleIcon?.setAttribute('href', '/assets/gerrys-iglu-icon-512.png?v=ecosystems-app-88');
    favicon?.setAttribute('href', '/assets/gerrys-iglu-icon-512.png?v=ecosystems-app-88');
  }, [isGameApp]);
