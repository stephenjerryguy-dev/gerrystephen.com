...
function holderCount(owner) {
  ...
  return Number(owner?.tokenCount || owner?.count || ownership.tokenCount || ownership.token_count || ownership.totalCount || 0) || 0;
}

async function fetchOwners(contract) {
  const params = new URLSearchParams({
    collection: contract,
    limit: '100',
  });
  const response = await fetch(`${RESERVOIR_API}/owners/v2?${params.toString()}`, {
  ...
  return (data.owners || [])
    .map((owner) => ({
      address: owner.address || owner.owner,
...
export default async function handler(req, res) {
  ...
  try {
    const ownerLists = await Promise.allSettled([
      fetchOwners(SAPPY_SEALS_CONTRACT),
      fetchOwners(STAKED_SEALS_CONTRACT),
    ]);
